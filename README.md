...
想要將 **Code2Course** 引入您的任何專案，有三種方式：

### 選項 A：一鍵可分發包 (最簡單，適合非開發者)

下載預先打包好的 ZIP 檔案，解壓後直接使用：

**Mac 用戶**：雙擊 `run_mac.command`
**Windows 用戶**：雙擊 `run_windows.bat`
...
**進階選項**：使用 PyInstaller 生成單一可執行檔案

```bash
./build_exe.sh  # 生成 code2course 可執行檔案
```

這樣用戶只需要一個檔案就能運行，無需任何 Python 環境。

### 選項 B：使用 Git Submodule (推薦開發者)
...
1. 將 `code2course/` 裡面的 `run.sh` 複製到**外層專案根目錄**。
2. 將 `code2course/code2course_config.example.json` 複製到外層，並改名為 `code2course_config.json`。
